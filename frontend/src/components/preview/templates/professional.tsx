    ...
    >
      ...
      <div className="mb-6 text-center">
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(90deg, transparent 0%, #1e3a5f 20%, #1e3a5f 80%, transparent 100%)',
          }}
        />
      </div>
      ...
      <div className="space-y-1.5">
        {categories.map((cat: any) => (
          <div key={cat.id} data-pdf-item className="flex text-sm">
            <span className="w-32 shrink-0 font-semibold" style={{ color: '#1e3a5f' }}>
              {cat.name}:
            </span>
      ...
      <div className="space-y-1.5">
        {items.map((item: any) => (
          <div key={item.id} className="flex items-baseline justify-between text-sm">
            ...
            {item.date && (
              <span className="shrink-0 text-xs text-zinc-400 italic">{item.date}</span>
            )}
          </div>
        ))}
      </div>
