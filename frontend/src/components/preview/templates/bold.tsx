      ...
      <div className="bg-black px-8 py-8 text-white">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            {pi.jobTitle && (
              <p className="mt-1 text-lg font-light text-zinc-400">
                {pi.jobTitle}
              </p>
            )}
          </div>
        ...
        {resume.sections
          ...
          .map((section) => (
            <div key={section.id} className="mb-6" data-section>
              <h2 className="mb-3 pb-2 border-b-4 border-black text-lg font-black uppercase tracking-wider text-black">
                {section.title}
              </h2>
              <div className="mt-2">
            ...
            {item.technologies?.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {item.technologies.map((t: string, i: number) => (
                  <span
                    key={i}
                    className="border border-zinc-300 px-2 py-0.5 text-[10px] font-medium text-zinc-500"
                  >
                    {t}
                  </span>
                ))}
      ...
      <div className="flex flex-wrap gap-2">
        {(content.categories || []).flatMap((cat: any) =>
          (cat.skills || []).map((skill: string, i: number) => (
            <span
              key={`${cat.id}-${i}`}
              className="border-2 border-black px-3 py-1 text-xs font-bold text-black"
            >
              {skill}
            </span>
          )),
