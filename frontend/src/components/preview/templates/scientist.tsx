  ...
  return (
    ...
    >
      ...
      <div className="mb-6 text-center">
        ...
        <div
          className="mt-2 flex flex-wrap items-center justify-center gap-3 text-xs"
          style={{ color: MUTED }}
        >
...
function ScientistSectionContent({
  ...
  if (section.type === "work_experience") {
    return (
      <div className="space-y-4">
        {((content as WorkExperienceContent).items || []).map(
          (item: any, idx: number) => (
            <div key={item.id}>
              ...
              {item.technologies?.length > 0 && (
                <p className="pl-6 text-xs italic" style={{ color: MUTED }}>
                  {resume.language === "zh" ? "技术栈" : "Methods/Tools"}:{" "}
  ...
  if (section.type === "education") {
    return (
      <div className="space-y-3">
        {((content as EducationContent).items || []).map(
          (item: any, idx: number) => (
            <div key={item.id}>
              <div className="flex items-baseline justify-between">
                <div>
              ...
              {item.gpa && (
                <p className="pl-6 text-xs" style={{ color: MUTED }}>
                  GPA: {item.gpa}
                </p>
              )}
              {item.highlights?.length > 0 && (
      ...
      <div className="space-y-1.5">
        {((content as SkillsContent).categories || []).map((cat: any) => (
          <div key={cat.id} className="text-sm">
            <span className="font-bold italic" style={{ color: PRIMARY }}>
              {cat.name}:{" "}
            </span>
  ...
  if (section.type === "projects") {
    return (
      <div className="space-y-3">
        {((content as ProjectsContent).items || []).map(
          (item: any, idx: number) => (
            <div key={item.id}>
              <div className="flex items-baseline justify-between">
                <div>
  ...
  if (section.type === "certifications") {
    return (
      <div className="space-y-1.5">
        {((content as CertificationsContent).items || []).map(
          (item: any, idx: number) => (
            <div key={item.id} className="text-sm">
              <span className="text-xs font-bold" style={{ color: ACCENT }}>
                [{idx + 1}]
              </span>
              <span className="ml-1.5 font-medium" style={{ color: PRIMARY }}>
                {item.name}
              </span>
      ...
      <div className="space-y-3">
        {items.map((item: any, idx: number) => (
          <div key={item.id}>
            ...
            {item.language && (
              <span className="pl-6 text-xs italic" style={{ color: ACCENT }}>
                {item.language}
              </span>
            )}
            {item.description && (
  ...
  if (section.type === "custom") {
    return (
      <div className="space-y-3">
        {((content as CustomContent).items || []).map(
          (item: any, idx: number) => (
            <div key={item.id}>
              <div className="flex items-baseline justify-between">
                <div>
