    ...
    >
      ...
      <div className="flex">
        <div className="flex-1 px-8 py-8">
          ...
          {pi.jobTitle && (
            <p className="mt-1 text-lg font-light" style={{ color: CORAL }}>
              {pi.jobTitle}
            </p>
          )}
        ...
        {pi.avatar && <AvatarImage src={pi.avatar} size={128} wrapperClassName="w-32 shrink-0" />}
      </div>
...
function DesignerSectionContent({ section, resume }: { section: any; resume: Resume }) {
  ...
  if (section.type === 'summary') {
    return (
      <p
        className="border-l-4 pl-4 text-sm leading-relaxed text-zinc-600"
      ...
      <div className="space-y-4">
        {(content.items || []).map((item: any) => (
          <div key={item.id} className="rounded-lg bg-zinc-50 p-4">
            ...
            {item.company && (
              <p className="text-sm font-medium" style={{ color: CORAL }}>
                {item.company}
              </p>
            )}
            {item.description && (
      ...
      <div className="space-y-3">
        {items.map((item: any) => (
          <div key={item.id} data-pdf-item className="rounded-lg bg-zinc-50 p-4">
            ...
            {item.language && (
              <span className="text-xs font-medium" style={{ color: CORAL }}>
                {item.language}
              </span>
            )}
            {item.description && (
