...
export function AcademicTemplate({ resume }: { resume: Resume }) {
  ...
  return (
    <div
      ...
      style={{
        fontFamily:
          '"Computer Modern", "CMU Serif", Georgia, "Times New Roman", serif',
      }}
    >
      ...
      <div className="mb-6">
        <div className={pi.avatar ? "flex items-center gap-4" : "text-center"}>
          ...
          <div>
            ...
            {pi.jobTitle && (
              <p className="mt-0.5 text-base text-zinc-700 italic">
                {pi.jobTitle}
              </p>
            )}
            {contacts.length > 0 && (
      ...
      {resume.sections
        ...
        .map((section) => (
          <div key={section.id} className="mb-6 pt-1" data-section>
            <h2
              className="mb-3 pb-2 text-xs font-bold uppercase tracking-[0.25em] text-zinc-800"
...
function AcademicSectionContent({
  ...
  if (section.type === "summary") {
    return (
      <p
        className="text-sm leading-relaxed text-zinc-600 indent-8"
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.field && (
                  <span className="text-sm text-zinc-600"> - {item.field}</span>
                )}
              </div>
              <span className="shrink-0 text-xs text-zinc-500">
                {item.startDate} –{" "}
