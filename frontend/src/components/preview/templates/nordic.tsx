  ...
  return (
    ...
    >
      ...
      <div className="mb-8 text-center">
        ...
        <h1 className="text-2xl font-light tracking-wide" style={{ color: SLATE_500 }}>
          {pi.fullName || 'Your Name'}
        </h1>
        {pi.jobTitle && (
          <p className="mt-1 text-sm font-light tracking-wider" style={{ color: SLATE_400 }}>
            {pi.jobTitle}
          </p>
        )}
        <div
        ...
        {((content as WorkExperienceContent).items || []).map((item: any) => (
          <div key={item.id} className="rounded-sm p-3" style={{ backgroundColor: SLATE_50 }}>
            <div className="flex items-baseline justify-between">
              <div>
                <span className="text-sm font-medium" style={{ color: SLATE_500 }}>
                  {item.position}
                </span>
                {item.company && (
                  <span className="text-sm font-light" style={{ color: SLATE_400 }}>
                    {' '}
                    | {item.company}
                  </span>
                )}
      ...
      <div className="space-y-3">
        {((content as EducationContent).items || []).map((item: any) => (
          <div key={item.id} className="rounded-sm p-3" style={{ backgroundColor: SLATE_50 }}>
            ...
            {item.gpa && (
              <p className="text-xs font-light" style={{ color: SLATE_400 }}>
                GPA: {item.gpa}
              </p>
            )}
            {item.highlights?.length > 0 && (
      ...
      <div className="space-y-1.5">
        {((content as SkillsContent).categories || []).map((cat: any) => (
          <div key={cat.id} className="flex text-sm">
            <span className="w-28 shrink-0 font-medium" style={{ color: SLATE_500 }}>
              {cat.name}:
            </span>
            <span className="font-light" style={{ color: SLATE_400 }}>
              {(cat.skills || []).join(', ')}
            </span>
          </div>
        ))}
        ...
        {items.map((item: any) => (
          <div key={item.id} className="rounded-sm p-3" style={{ backgroundColor: SLATE_50 }}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-xs font-light" style={{ color: SLATE_400 }}>
