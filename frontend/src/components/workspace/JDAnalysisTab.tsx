...
export default function JDAnalysisTab({
  ...
  const record = useMemo(
    () => enrichedList.find((a) => a.id === effectiveSelectedId) || enrichedList[0] || null,
  ...
  const keywordMatchesArr =
    Array.isArray(record.keywordMatches) && typeof record.keywordMatches[0] === 'object'
      ? (record.keywordMatches as any[])
      : [];
  const partialMatchesArr = record.partialMatches || [];
  const missingKeywordsArr =
    Array.isArray(record.missingKeywords) && typeof record.missingKeywords[0] === 'object'
  ...
  const renderKeywordContent = () => {
    if (activeKwTab === 'matched') {
      if (!keywordMatchesArr.length)
        return (
          <div className="text-center py-8 text-muted-foreground text-sm">暂无完全匹配的关键词</div>
        );
    ...
    if (activeKwTab === 'partial') {
      if (!partialMatchesArr.length)
        return <div className="text-center py-8 text-muted-foreground text-sm">暂无部分匹配项</div>;
    ...
    if (!missingKeywordsArr.length)
      return <div className="text-center py-8 text-muted-foreground text-sm">暂无缺失关键词</div>;
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.suggestion && (
              <div className="text-[10px] text-muted-foreground bg-red-500/5 border border-red-500/10 rounded px-2 py-1.5 break-all">
                <Lightbulb className="w-3 h-3 inline-block mr-1" /> {item.suggestion}
              </div>
            )}
          </div>
        </div>
  ...
  const getRecordCounts = (a: JdAnalysis) => {
    ...
    const missingKw =
      Array.isArray(a.missingKeywords) && typeof a.missingKeywords[0] === 'object'
            ...
            {enrichedList.map((a, idx) => {
              ...
              return (
                <div key={a.id}>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-baseline gap-1.5">
                        <span className={`text-sm font-bold ${sc.text}`}>{a.overallScore}</span>
                  ...
                  {idx < enrichedList.length - 1 && <div className="border-b border-border/50" />}
                </div>
              );
            })}
          </div>
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-3 mb-3 text-[10px] text-muted-foreground">
                        <span className="flex items-center gap-1">
                          <Square className="w-2.5 h-2.5 text-pink-500" /> 硬性要求
                        </span>
                        <span className="flex items-center gap-1">
                          <Square className="w-2.5 h-2.5 text-amber-400" /> 优先项
                        </span>
                        <span className="flex items-center gap-1">
                          <Square className="w-2.5 h-2.5 text-slate-400" /> 加分项
                        </span>
                      </div>
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      {req.level && (
                                        <span className={`text-[9px] ${style.text} opacity-80`}>
                                          {req.level}
                                        </span>
                                      )}
                                    </div>
              ...
              <div className="rounded-xl border border-border bg-card p-5">
                <div className="flex items-center justify-between mb-4 min-w-0 overflow-x-hidden">
                  ...
                  <div className="flex items-center gap-3 text-[10px] shrink-0">
                    ...
                    <span className="flex items-center gap-1">
                      <Square className="w-2.5 h-2.5 text-yellow-500" /> 中优先级
                    </span>
                    <span className="flex items-center gap-1">
                        ...
                        <div className="p-4">
                          <div className="flex items-center gap-2 mb-3 min-w-0 overflow-x-hidden">
                            <span className="text-sm shrink-0">
                              {typeIcons[s.type] || <Lightbulb className="w-3 h-3" />}
                            </span>
                            ...
                            {s.targetDimension && (
                              <span className="text-[9px] text-muted-foreground break-all min-w-0">
                                影响：
                                {dimLabels[s.targetDimension] || s.targetDimension}
                              </span>
                            )}
