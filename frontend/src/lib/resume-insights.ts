...
export function calculateHealthScore(
  ...
  const wordCounts = visibleSections.map(
    (s) => getTextContent(s).trim().length,
  );
  const completedCount = visibleSections.filter(
    (s) => getTextContent(s).trim().length > 0,
  ).length;
  const completionScore = Math.min(completedCount * 8, 40);

  const avgCount = wordCounts.reduce((a, b) => a + b, 0) / wordCounts.length;
  const variance =
    wordCounts.reduce((sum, count) => sum + Math.pow(count - avgCount, 2), 0) /
    wordCounts.length;
