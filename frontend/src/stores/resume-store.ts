...
export const useResumeStore = create<ResumeStore>((set, get) => ({
  ...
  updateSection: (sectionId, content) => {
    set((state) => {
      const sections = state.sections.map((s) =>
        s.id === sectionId
          ? { ...s, content: { ...s.content, ...content } as SectionContent }
          : s,
      );
  ...
  setThemeConfig: (themeConfig: Partial<ThemeConfig>) => {
    ...
    set((state) => ({
      currentResume: state.currentResume
        ? {
            ...state.currentResume,
            themeConfig: { ...state.currentResume.themeConfig, ...themeConfig },
          }
        : null,
    }));
    if (currentResume?.id) {
      updateResume({
        id: currentResume.id,
        themeConfig: { ...currentResume.themeConfig, ...themeConfig },
      });
    }
  },
  ...
  _scheduleSave: () => {
    ...
    const { autoSave, autoSaveInterval, _hydrated } =
